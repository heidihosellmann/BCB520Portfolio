...
**Head to @fig-ClassNetwork for my final draft of our BCB520 class network data!**
...
```{r}
#| label: fig-ClassNetwork
#| fig-cap: "Class Network Figure"
...
dept_colors <- c("blue", "red", "green", "orange", "purple", "yellow") # Add more colors as needed

# Assign colors to vertices based on department
V(g)$color <- dept_colors[match(V(g)$department, unique(people$department))]

# Define colors for edges
edge_colors <- c("navyblue", "darkgray")
E(g)$color <- ifelse(E(g)$BCB520 == "TRUE", edge_colors[1], edge_colors[2])

# Set edge width for "BCB520" connections
E(g)$width <- ifelse(E(g)$BCB520 == "TRUE", 1.5, 1.1) # You can adjust the width value as needed
...
plot(
  ...
  vertex.label.color = "black", # Set label color to black
  main = "Network of DataViz Students and Advisors"
)

# Create legend using the same colors
legend("bottomright", legend = unique(people$department), fill = dept_colors, title = "Department")

# Create legend for edges
legend("bottomleft", legend = c("BCB520", "Advisor"), col = edge_colors, lty = 1, title = "Edge Type")
