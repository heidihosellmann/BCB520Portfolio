...
**Figure 1.** is an example of what I want to expand to all the data
...
**Figure 2.** is a similar premise to Figure 1, however, now I am
coloring by sponsor.
...
Alrighty, now time to bind all the new data sets together.

## Putting all the active awards from various sponsors together

```{r Visualizing new data frames from all sponsors}
...
ggplot(data = combined_active_grants_df, aes(x = Start_Date, xend = End_Date, y = PI, yend = PI, color = Sponsor, alpha = Amount_Awarded_to_Date)) +
  geom_segment(size = 3) +  # Add segments for each grant
...
**Figure 3.** shows the PIs of currently active awards (to the best of
...
**Figure 4.** captures the number of new grants the DOE awarded to the
...
**Figure 5.** shows the approximate trends of new grants received by the
...
on the board will start the ball rolling again.
...
**Figure 6.** shows the award amounts of the active grants of each
